...
arma::vec INTERPLIN( arma::vec &yData, arma::vec &zData, arma::vec yPred )
{
	...
	for(int i = 0; i < size; ++i) {
        ...
        // checa se o ponto interpolado esta de fato dentro dos limites
        bool inrangey = (yPred[i] >= yData[0]) && (yPred[i] <= *(yData.end() - 1));
    ...
    arma::vec &xPred, arma::vec &yPred )
{
	...
	for(int i = 0; i < size; ++i) {
        ...
        // checa se o ponto interpolado esta de fato dentro dos limites
        bool inrangex = (xPred[i] >= xData[0]) && (xPred[i] <= *(xData.end() - 1));
        bool inrangey = (yPred[i] >= yData[0]) && (yPred[i] <= *(yData.end() - 1));
